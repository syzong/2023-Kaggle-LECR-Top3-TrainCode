...
#valid_topic_id = pd.read_csv('data/valid_samples_1k_second.csv')
...
df_labels = pd.concat([df_labels, df_target_pseudo])
...
if train_flag:
    df_train.to_csv('train_df_sbert_first.csv', index=None)
else:
    df_train.to_csv('dev_df_sbert_first.csv', index=None)
